...
class FrameImagesView: UIScrollView {
    ...
    var timeTolerance = CMTimeMakeWithSeconds(10 , preferredTimescale:1000)
    ...
    func stopAnimation() {
        ...
        if let parent = parentScroller {
            frame.origin = CGPoint(x: TimelineScroller.HorizontalSpacing, y: parent.measureHeight)
    ...
    func displayFrames() {
        ...
        if let parent = parentScroller {
            let visibleHalf = TimelineScroller.HorizontalSpacing
            ...
            if animating {
                if let layer = parent.layer.presentation() {
                    ...
                    frame.origin = offset
                    print("trimview set origin = \(offset)")
    ...
    func coordinateFrames() -> (upon:Set<Int>, below:Set<Int>, hidden:Set<Int>, deep:Set<Int>) {
        ...
        let keyDivision = 1 //Int(pow(2,Double(Int(log2(maxWidth * 2 / (self.frame.size.width + 1))))))
        if keyDivision <= 0 {
        ...
        print("trimview keyDivision \(keyDivision) keyCount \(keyCount) imagesCount = \(frameImagesArray.count)")
        ...
        return (uponElements, Set<Int>(), Set<Int>(), Set<Int>())
    ...
    func setThumnailFrameSize() {
        ...
        //    thumbnailFrameSize = CGSize(width: (mainView.frame.width - TimelineScroller.HorizontalSpacing * 2) / 6, height: 50.cg)
    }
    ...
    func thumbnailCountFloat() -> CGFloat {
        return maxWidth * mainView.timelineView.videoScale / thumbnailFrameSize.width
    }
    ...
    func indexOfVisibleSide() -> (left:Int, right:Int) {
        ...
        let leftSide = -(parentScroller!.frame.width / 2) + parentScroller!.contentOffset.x - thumbnailFrameSize.width
        let rightSide = parentScroller!.contentOffset.x + (parentScroller!.frame.size.width) + thumbnailFrameSize.width
        var visibleLeft = indexWithPosition(-(parentScroller!.frame.width / 2) + parentScroller!.contentOffset.x - thumbnailFrameSize.width) - 1
        var visibleRight = indexWithPosition(parentScroller!.contentOffset.x + (parentScroller!.frame.size.width) + thumbnailFrameSize.width)
        
        print("visible left = \(visibleLeft) visible right = \(visibleRight) leftSide \(leftSide) rightSide \(rightSide)")
        ...
        if (visibleRight - visibleLeft > 40) { visibleRight = visibleLeft + 40} //to prevent possible bugs
    ...
    func requestImageGeneration(timesArray:[NSValue]) {
        ...
        if let asset = mainView!.asset {
            ...
            assetImgGenerate.generateCGImagesAsynchronously(forTimes: timesArray,
                ...
                //print("generate image for time \(timeValue)")
