...
const float DISPLACEMENT_PIXEL_STEP = 12.0;
const float GREEN_DISPLACEMENT_PIXEL_STEP = 12.0;
...
varying vec2 vTextureCoord_6_;// Green displacement mask
...
void main() {
    ...
    float greenDisplacementValue = texture2D(sTexture_1_, vTextureCoord_6_).r;
    vec2 rbCoord = displacement(vTextureCoord_0_, uPixelSize_0_, displacementMask, DISPLACEMENT_PIXEL_STEP, DISPLACEMENT_PIXEL_STEP);
    vec2 gCoord = displacement(rbCoord, uPixelSize_0_, vec2(greenDisplacementValue, greenDisplacementValue), GREEN_DISPLACEMENT_PIXEL_STEP, -GREEN_DISPLACEMENT_PIXEL_STEP);
    vec4 source = clampToBorder(rbCoord);
    source.g = clampToBorder(gCoord).g;
