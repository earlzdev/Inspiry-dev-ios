...
void main() {
    ...
    vec4 overlay = texture2D(sTexture_1_, vTextureCoord_1_);
    vec4 textMask = texture2D(sTexture_2_, vTextureCoord_2_);

    vec4 maskedOverlay = mix(overlay, vec4(0.0, 0.0, 0.0, 0.0), textMask.a);
    vec4 result = mix(source, maskedOverlay, maskedOverlay.a);
