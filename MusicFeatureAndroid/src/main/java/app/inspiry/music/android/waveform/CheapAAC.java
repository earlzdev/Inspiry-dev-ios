...
public class CheapAAC extends CheapSoundFile {
    ...
    public int getAvgBitrateKbps() {
        return (int) (getFileSizeBytes() / (mNumFrames * mSamplesPerFrame));
    }
    ...
    @Override
    public void readStream(InputStream inputStream, long fileSize)
            throws java.io.IOException {

        inputStream = new BufferedInputStream(inputStream);
        super.readStream(inputStream, fileSize);
        ...
        // Read the first_image 8 bytes
        byte[] header = new byte[8];
        inputStream.mark(1024 * 1024 * 10);
        ...
        if (header[0] == 0 &&
                ...
                header[6] == 'y' &&
                header[7] == 'p') {
            // Create a new inputStream, reset to the beginning of the file
            inputStream.reset();
            parseMp4(inputStream, (int) fileSize);
        } else {
        ...
        if (mMdatOffset > 0 && mMdatLength > 0) {
            inputStream.reset();
            inputStream.skip(mMdatOffset);
            mOffset = mMdatOffset;
            parseMdat(inputStream, mMdatLength);
        } else {
