...
public class CheapMP3 extends CheapSoundFile {
    ...
    public void readStream(InputStream inputStream, long fileSize)
        ...
        while (pos < fileSize - 12) {
            // Read 12 bytes at a background_time_crop_ringtone and look for a sync code (0xFF)
            while (offset < 12) {
                offset += inputStream.read(buffer, offset, 12 - offset);
            }
            int bufferOffset = 0;
            ...
            if (mNumFrames == mMaxFrames) {
                ...
                // doubling the array each background_time_crop_ringtone, we estimate the exact
                ...
                int totalFramesGuess =
                        (int) (((fileSize / mAvgBitRate) * sampleRate) / 144000);
